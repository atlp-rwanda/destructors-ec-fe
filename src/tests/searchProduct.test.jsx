...
describe("createProduct Form", () => {
  ...
  let productsMock = { products: [{
    ...
    images: 'https://res.cloudinary.com/dboqnapgi/image/upload/v1685454205/OurProject/efvw84uvxjxz67qbvgms.png',
  }]};
  ...
  it("renders searchInputField", () => {
    ...
    mock.onGet(`${import.meta.env.VITE_REACT_APP_API_URL}/products/search`).reply(200, productsMock);
  ...
  it("renders searchSuggestion", async () => {
    ...
    await userEvent.type(screen.getByPlaceholderText("search for products"), "t");
  ...
  it("should search product", async () => {
    ...
    await userEvent.type(screen.getByPlaceholderText("search for products"), "television");
    await userEvent.click(screen.getByDisplayValue("Filters"));
    await userEvent.selectOptions(screen.getByRole("combobox"), "bags");
    await userEvent.type(screen.getByPlaceholderText("Maximum price"), "3000");
    await userEvent.click(screen.getByTestId("searchButton"));
    expect(screen.getByText('Quantity: 20 pieces')).toBeInTheDocument();
  });
  it("should search product by date of expiry", async () => {
    ...
    await userEvent.click(screen.getByDisplayValue("Filters"));
    await userEvent.type(screen.getByPlaceholderText("2022/06/10"), "2023-06-29");
    await userEvent.click(screen.getByTestId("searchButton"));
    expect(screen.getByText('television')).toBeInTheDocument();
  });
  it("should return No product found", async () => {
    ...
    mock.onGet(`${import.meta.env.VITE_REACT_APP_API_URL}/categories`).reply(200, categoriesMock);
    mock.onGet(`${import.meta.env.VITE_REACT_APP_API_URL}/products/search`).reply(200, productMock);
    ...
    await userEvent.type(screen.getByPlaceholderText("search for products"), "tourch");
    await userEvent.click(screen.getByDisplayValue("Filters"));
