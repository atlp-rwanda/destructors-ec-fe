...
function NavBar() {
  ...
  useEffect(() => {
    if (info?.data?.id) dispatch(getProductWishilist());
  }, []);
  ...
  useEffect(() => {
    const newSocket = io(
      `${import.meta.env.VITE_REACT_APP_API_URLs}/notifications`,
      {
        extraHeaders: {
      ...
      {info?.data?.role ? (
        <div className='  flex gap-2 cursor-pointer'>
          ...
          <div>
            ...
            {openNotification && <Notification notifications={notifications} />}
