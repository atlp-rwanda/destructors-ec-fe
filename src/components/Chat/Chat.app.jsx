...
const ChatApp = ({ loggedInUser }) => {
  ...
  useEffect(() => {
    ...
    newSocket.on('connect', () => {
      ...
      newSocket.emit('newuser', loggedInUser);
    });
    ...
    newSocket.on('getOldMessages', (messagesData) => {
      const messages = messagesData.map((message) => ({
        userId: message.isMine ? loggedInUser : message.senderId,
    ...
    return () => {
      newSocket.emit('exituser', loggedInUser);
  ...
  const handleChatSubmit = async () => {
    if (message.trim() !== '') {
      ...
      setOldMessages((prevMessages) => [...prevMessages, { type: 'my', ...newMessage }]);
    ...
    return oldMessages.map((message, index) => (
      <div
        className={`px-[12px] text-[12px] ${
          message.userId === loggedInUser ? 'bg-[#a4a1a1]  ml-auto rounded-tl-[25px] rounded-tr-[25px] rounded-bl-[25px]' : 'bg-[#f7f7f7] rounded-tl-[25px] rounded-tr-[25px] rounded-br-[25px]'
        } w-fit shadow-md`}
        key={index}
  ...
  return (
    <div className="flex flex-col-reverse content-center mt-[30%] w-full">
      {chatVisible && (
        <div id="main" className="fixed flex flex-col h-[60vh] gap-2 w-[50%] xs:w-[93%] overflow-x-hidden shadow-2xl rounded-tl-[25px] rounded-tr-[25px] rounded-br-[25px] bg-[#ffffff]  mt-auto mr-auto">
          <div className="bg-[#103547] text-[white] h-[10%] p-[25px] flex items-center">
            <h1>Chat</h1>
          </div>
          <div id="body" className="p-[20px] flex flex-col gap-2 flex-grow overflow-scroll bg-[#ffffff]">
            {renderOldMessages()}
          </div>
          <div className="w-[80%] border-[1px] border-[#dad6d6] relative flex rounded-md mb-7 self-center">
            <input
              ...
              className="w-3/4 py-2 px-3 rounded-l-md text-[gray] text-xs"
              value={message}
              onChange={(e) => {
            ...
            />
            <button className="w-1/4 bg-white py-2 px-3 rounded-r-md" onClick={handleChatSubmit}>
              <img className="h-5 ml-auto text-white w-5" src={send} alt="Send" />
            </button>
          </div>
        </div>
      )}
      <button onClick={handleChatVisible} className="fixed ">
        {' '}
        {chatVisible ? (
        <img className="h-10 p-[6px] ml-auto text-white w-10" src={cross} alt="" />
        ):
        (
